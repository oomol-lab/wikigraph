...
wikigraph wkg://book.wikg/chunk --query "keyword"
wikigraph wkg://book.wikg/entity --query "keyword"
wikigraph wkg://book.wikg/chapter/3/source#0..8
...
优先先选择三种探索模式之一。对于综合理解、时间线、关系分析、过程梳理或概念结构任务，先走结构模式：用 `wkg://.../chapter/tree` 查看压缩后的目录地图，再选择可能相关的 chapter id，并用 scoped URI 加 `--query` 或对象 URI 展开局部。搜索模式使用 scope URI，例如 `wkg://.../chunk --query <query>` 或 `wkg://.../entity --query <query>` 做候选定位。阅读模式适合在选定 source URI 后直接输出连续文本。
...
在 URI 中显式选择 scope 或 object：`/chunk` 用于 Reading Graph 结构，`/entity` 和 `/triple` 用于 Knowledge Graph 对象；`/source` 和 `/summary` 是可读对象，默认读取内容。Lens 位置决定 scope：用 `<archive-uri>/entity` 枚举整本归档的 entity；只有需要单章 entity 时，才用 `<chapter-uri>/entity`。使用 `wkg://book.wikg/chapter/3/entity` 这类 scoped chapter lens URI 控制检索范围。
